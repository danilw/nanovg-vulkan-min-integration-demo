...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
  ...
  for (i = 0; i < 12; i++) {
    char file[128];
    snprintf(file, 128, "nanovg_vulkan_resources/images/image%d.jpg", i+1);
  ...
  data->fontIcons = nvgCreateFont(vg, "icons", "nanovg_vulkan_resources/font/entypo.ttf");
  if (data->fontIcons == -1) {
    printf("Could not add font icons.\n");
    return -1;
  }
  data->fontNormal = nvgCreateFont(vg, "sans", "nanovg_vulkan_resources/font/Roboto-Regular.ttf");
  if (data->fontNormal == -1) {
    printf("Could not add font italic.\n");
    return -1;
  }
  data->fontBold = nvgCreateFont(vg, "sans-bold", "nanovg_vulkan_resources/font/Roboto-Bold.ttf");
  if (data->fontBold == -1) {
    printf("Could not add font bold.\n");
    return -1;
  }
  data->fontEmoji = nvgCreateFont(vg, "emoji", "nanovg_vulkan_resources/font/NotoEmoji-Regular.ttf");
  if (data->fontEmoji == -1) {
